...
class MixerMSE(nn.Module):
    ...
    def forward(self, x, target):
        ...
        loss = self.criterion1(x[0, 0, :], target[0, 0, :]) + self.criterion2(
            x[0, 1, :], target[0, 1, :]
        )
...
def cal_si_snr_with_pit(source, estimate_source, source_lengths):
    ...
    # # ==============================================================================================
    # print(estimate_source.size() == source.size())
    # exit()
    assert source.size() == estimate_source.size()
    B, C, T = source.size()  # get all parameters
    # mask padding position along T
    ...
    perms_one_hot = (
        source.new_zeros((*perms.size(), C)).scatter_(2, index, 1).type(torch.float)
    )
    # print("perms_one_hot", perms_one_hot.type())
    # [B, C!] <- [B, C, C] einsum [C!, C, C], SI-SNR sum of each permutation
    snr_set = torch.einsum("bij,pij->bp", [pair_wise_si_snr, perms_one_hot])
